...
# Project SCS 18 patients Experiment Denmark

# monocle on SMCs and macrophages

#-----------------------------------------------------------------------------------------
### Settings

# set directory
setwd("C:/Users/lslender/Documents/Projecten/Project SCS 18 patients/Experiment Denmark")
...
# object
seuset <- readRDS("SMCs and MCs 18 patients.RDS")

#-----------------------------------------------------------------------------------------
### get cells
# original data
seuset <- readRDS("seuset 18 patients 16 communities.RDS")

# subset
seubset <- SubsetData(seuset,
                      ident.use = c(2, 7, 8, 12))
# find variable genes
...
# plot
pdf(paste(result.folder, "/", Sys.Date(), "Monocle SMCs and MCs 18 patients res 1.pdf", sep = ""))

TSNEPlot(seubset, do.label = T, pt.size = 1, label.size = 5, group.by = "ident", plot.title = "identity")
TSNEPlot(seubset, do.label = T, pt.size = 1, label.size = 5, group.by = "res.1.7", plot.title = "res1.7")
...
# save subset
saveRDS(seubset, "SMCs and MCs 18 patients.RDS")
...
pd <- new("AnnotatedDataFrame", data = seuset@meta.data[colnames(expression.matrix), c(1,2,19,20)])
...
qplot(Total_mRNAs, data = pData(dandy), color = res.1, geom =
        "density") +
...
dandy <- reduceDimension(dandy, 
                         ...
                         #residualModelFormulaStr = "~ Patient + Phenotype + CD3.score + CD34.score + alpha.SMA.score + SR.score + Glyc.c.score",
                         #residualModelFormulaStr = "~ nGene + nUMI + Patient + Sex + alpha.SMA.score",
                         residualModelFormulaStr = "~ nGene + nUMI + num_genes_expressed",
                         norm_method = "vstExprs",
                         verbose = F)
...
# plot results
pdf(paste(result.folder, "/", Sys.Date(), " clustering SMCs and MCs.pdf", sep = ""))
...
dandy <- orderCells(dandy)
#plot
pdf(paste(result.folder, "/", Sys.Date(), " celltrajectory SMCs and MCs.pdf", sep = ""))
...
GM_state <- function(cds){
  if (length(unique(pData(cds)$State)) > 1){
    T0_counts <- table(pData(cds)$State, pData(cds)$res1)[,"3"]
...
saveRDS(dandy, "monocle SMCs MCs subset 18 patients.RDS")
